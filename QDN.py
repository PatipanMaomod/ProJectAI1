...
class Agent:
    def __init__(self, learning_rate=0.001, gamma=0.85, epsilon=1.0, input_dim=14,
...
async def echo(websocket):
    agent_id = str(uuid.uuid4())  # ให้แต่ละการเชื่อมต่อมี id
    connected_agents[agent_id] = {"websocket": websocket}
    agent = Agent()
    load_model(agent)
